...
if($_SERVER["REQUEST_METHOD"] == "GET"){
  ...
  if(!empty($_GET['c_code_filter']) && !empty($_GET['sort_records'])){
    ...
    $sql = "select name, position, salary from salarydealing where position like '%".$_GET['c_code_filter']."%'
    order by position $sql_desc_extension;";

  }elseif(empty($_GET['c_code_filter']) && !empty($_GET['sort_records'])){

    $sql = "select name, position, salary from salarydealing order by position $sql_desc_extension;";

  }elseif(!empty($_GET['c_code_filter']) && empty($_GET['sort_records'])){
    $sql = "select name, position, salary from salarydealing
  ...
  <body>
    <div class="main_content" align="center">
      <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
        ...
        <button><a href="HR.php">Dashboard</a></button>
      </form>
    </div>
  </body>
</html>
